...
const Drop = () => {
  ...
  const handleFilter = () => {
    if (filterDate) {
      const filtered = predictions.filter((prediction) => prediction.Date === filterDate);
      setFilterResults(filtered);
    } else {
      setFilterResults(predictions); // If no filter date, reset to all predictions
    }
  };

  const handleClearFilter = () => {
    setFilterDate('');
    setFilterResults(predictions);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Error: Could not fetch predictions.");
    }
  };

  const commodityOptions = Object.keys(data);
  const stateOptions = commodity ? Object.keys(data[commodity]) : [];
  const districtOptions = state ? Object.keys(data[commodity][state]) : [];
  const marketOptions = district ? data[commodity][state][district] : [];

  const prepareChartData = (minPrices, maxPrices, modalPrices, dates) => {
    return {
      series: [
        {
          name: 'Min Price',
      ...
      options: {
        ...
        title: {
          text: 'Price Predictions (Min, Max, Modal)',
          align: 'center'
        }
      }
    };
  ...
  const renderVisualizations = () => {
    if (!predictions || predictions.length === 0) {
      return <div>No predictions available. Please make sure all fields are filled out correctly.</div>;
    }

    const minPrices = predictions.map((prediction) => prediction["Min Price"]);
    const maxPrices = predictions.map((prediction) => prediction["Max Price"]);
    const modalPrices = predictions.map((prediction) => prediction["Modal Price"]);
    const dates = predictions.map((prediction) => prediction.Date);

    return (
      <div className="visualization-container">
        <div className="grid-layout">
          {/* Plot 1: Line chart for price predictions by district */}
          <div className="chart-container">
            <h3>Plot 1: Price Predictions by District (ApexChart)</h3>
            <ReactApexChart
              options={prepareChartData(minPrices, maxPrices, modalPrices, dates).options}
              series={prepareChartData(minPrices, maxPrices, modalPrices, dates).series}
              type="line"
              height={350}
            />
          </div>

          {/* Plot 2: Bar chart for price predictions by district */}
          <div className="chart-container">
            <h3>Plot 2: Price Predictions by District (Bar Chart)</h3>
            <ReactApexChart
              options={prepareChartData(minPrices, maxPrices, modalPrices, dates).options}
              series={prepareChartData(minPrices, maxPrices, modalPrices, dates).series}
              type="bar"
          ...
          {/* Plot 3: Line chart for price predictions by market */}
          <div className="chart-container">
            <h3>Plot 3: Price Predictions by Market (Line Chart)</h3>
            <ReactApexChart
              options={prepareChartData(minPrices, maxPrices, modalPrices, dates).options}
              series={prepareChartData(minPrices, maxPrices, modalPrices, dates).series}
              type="line"
          ...
          {/* Plot 4: Bar chart for price predictions by market */}
          <div className="chart-container">
            <h3>Plot 4: Price Predictions by Market (Bar Chart)</h3>
            <ReactApexChart
              options={prepareChartData(minPrices, maxPrices, modalPrices, dates).options}
              series={prepareChartData(minPrices, maxPrices, modalPrices, dates).series}
              type="bar"
          ...
          {/* Plot 5: Line chart for price predictions by commodity */}
          <div className="chart-container">
            <h3>Plot 5: Price Predictions by Commodity (Line Chart)</h3>
            <ReactApexChart
              options={prepareChartData(minPrices, maxPrices, modalPrices, dates).options}
              series={prepareChartData(minPrices, maxPrices, modalPrices, dates).series}
              type="line"
          ...
          {/* Plot 6: Mixed chart: Line and Bar Chart together */}
          <div className="chart-container">
            <h3>Plot 6: Price Predictions (Mixed Line and Bar)</h3>
  ...
  return (
    <div className="main-container">
      <h2 className="header">Commodity Price Predictions</h2>

      <form onSubmit={handleSubmit} className="form-container">
        <div className="form-group">
          <label className="label">Select Commodity</label>
          <select value={commodity} onChange={handleCommodityChange} className="select-input">
        ...
        <div className="form-group">
          ...
          <select value={state} onChange={handleStateChange} className="select-input" disabled={!commodity}>
        ...
        <div className="form-group">
          ...
          <select value={district} onChange={handleDistrictChange} className="select-input" disabled={!state}>
        ...
        <div className="form-group">
          ...
          <select value={marketName} onChange={(e) => setMarketName(e.target.value)} className="select-input" disabled={!district}>
      ...
      <div className="filter-container">
        ...
        <button onClick={handleClearFilter} className="clear-btn">Clear Filter</button>
      </div>
  ...
  <table className="predictions-table">
    ...
    <tbody>
      {(filterResults.length > 0 ? filterResults : predictions).map((prediction, index) => (
        <tr key={index}>
          <td>{prediction.Date}</td>
          <td>{prediction["Min Price"]}</td>
          <td>{prediction["Max Price"]}</td>
          <td>{prediction["Modal Price"]}</td>
        </tr>
      ))}
    </tbody>
  </table>
