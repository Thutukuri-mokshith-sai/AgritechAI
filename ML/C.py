...
for file in files:
    try:
        ...
        # Create categorical labels for the price ranges
        y_min = pd.cut(df['Min Price (Rs./Quintal)'], bins=5, labels=False)  # Creating 5 categories for classification
        y_max = pd.cut(df['Max Price (Rs./Quintal)'], bins=5, labels=False)
        y_modal = pd.cut(df['Modal Price (Rs./Quintal)'], bins=5, labels=False)
        ...
        # Train SGDClassifier models
        min_price_model = SGDClassifier(loss='log', random_state=42)  # Using logistic regression loss
        max_price_model = SGDClassifier(loss='log', random_state=42)
        modal_price_model = SGDClassifier(loss='log', random_state=42)
